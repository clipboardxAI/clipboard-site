---
title: 生態系合作
description: 第三方 macOS 應用程式如何透過外送深層連結，接入 Clipboard x AI 的動作市集。
layout: doc
---

# 生態系合作

Clipboard x AI 是一個剪貼簿樞紐。但繁重的工作——檔案壓縮、圖片最佳化、PDF 處理——交由專門的同類應用程式來做會更好。**Action Marketplace（動作市集）** 已內建一個*第三方軟體*分類，其中的動作會直接委派給使用者已安裝的應用程式（[NueZip](https://github.com/w3cub/nuezip)、[Nicasa](https://github.com/w3cub/nicasa) 等）。
...
當使用者點擊 **Install（安裝）** 時，Clipboard x AI 會在本地註冊該動作。當他們在某個剪貼簿項目上觸發它時，應用程式會填入模板，並透過 `NSWorkspace.open(_:)` 開啟目標應用程式：
...
3. （選用）完成時呼叫 `returnURL`，讓 Clipboard x AI 能顯示「已在 &lt;App&gt; 中完成」的提示。
