...
- Clés API (dans le Keychain, uniquement si vous configurez un fournisseur cloud)
- Journaux de débogage (si activés)

Aucune de ces données n'est téléversée vers un serveur par l'application elle-même.

## Option d'IA cloud

Les fonctionnalités d'IA cloud sont **désactivées par défaut**. Elles n'envoient du contenu qu'au fournisseur que vous choisissez, en utilisant votre propre clé API, et uniquement lorsque vous déclenchez une action cloud. Vous pouvez consulter ou supprimer les fournisseurs configurés dans **Réglages → IA**.
